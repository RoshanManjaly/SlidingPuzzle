 ...
 * interface. Class contains a number of black box tests. Tests are very basic
 * in that they rely on as little extra functionality as possible aside from the
 * method under test. This implies that tests are not particularly elaborate and
 * may serve as a set of initial smoke tests to see if at least some basic
...
public class SimplePuzzleStateBasicsTest {
	 ...
	 * Empty slots are encoded with zero values. We test if this works for 1 empty
	 * slot. Test requires setToInitialState() and getValue() being properly
	 * implemented.
	 */
	@Test
	 ...
	 * The initial state for a given dimension and number of empty slots should list
	 * tiles with numbers in increasing order and have empty slots at the very end.
	 * Empty slots are encoded with zero values. We test if this works for several
	 * empty slots. Test requires setToInitialState() and getValue() being properly
	 * implemented.
	 */
	@Test
	 ...
	 *            state to analyze
	 * @param dim
	 *            dimension of board is (dim x dim)
	 * @param empties
	 *            the number of empty slots on the board
	 */
	 ...
	 * SimplePuzzleState class must implement (override) the equals method. If one
	 * does so, it is recommended good practice to also override the hashCode method
	 * to have a consistent class design. We check if equal states receive the same
	 * hashCode. Test requires setToInitialState() and hashCode() being properly
	 * implemented.
	 */
	@Test
	 ...
	 * The SimplePuzzleState class must implement (override) the equals method. Test
	 * requires setToInitialState() and equals() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getOperation()}. Trivial
	 * test to see if initial state returns null for the operation. Test requires
	 * getOperation() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getOperation()}. Test to see
	 * if direct successor state returns correct operation. Test requires
	 * getOperation() and move() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getParent()}. Trivial test
	 * to see if initial state returns null for the parent node. Test requires
	 * getParent() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getParent()}. Test to see if
	 * direct successor state returns the initial state as its parent. Test requires
	 * getParent(), equals() and move() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getPathLength()}. Test to
	 * see if the path length for the initial state is 0. Test requires
	 * getPathLength() being properly implemented.
	 */
	@Test
	 ...
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getPathLength()}. Test to
	 * see if the path length for a successor state of the initial state is 1. Test
	 * requires getPathLength() and move() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#shuffleBoard(int)}. Trivial
	 * test to see if shuffleBoard returns an equal state for length 0 and different
	 * states for length greater 0. Test requires setToInitialState(), equals(), and
	 * shuffleBoard() being properly implemented.
	 */
	@Test
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#getValue(int,int)}. Trivial
	 * test to see if shuffleBoard returns states where the sum of values across all
	 * tiles stays the same. Test uses getValue() extensively but only checks
	 * correctness with the help of an aggregate (sum). Test requires
	 * setToInitialState(), shuffleBoard() and getValue() being properly
	 * implemented.
	 */
	@Test
	 ...
	 * @param dim
	 *            the dimension of the board (dim x dim)
	 ...
	 * Test to see if a move operation works. We try two different directions for
	 * the same empty slot. We also check that the starting state remains intact.
	 * Test requires setToInitialState(), isEmpty(), equals() and move() being
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#drag(int,int,int,int)}. Test
	 * to see if a drag operation works. We try two different directions for the
	 * same empty slot. We also check that the starting state remains intact. Test
	 * requires setToInitialState(), isEmpty(), equals() and drag() being properly
	 * implemented.
	 */
	@Test
	public final void testDrag() {
		...
		// we will use this later to check that ps1 was not changed by the drag
		// operation
		assertEquals(ps1, ps3);
	...
	/**
	 * Test method for
	 * {@link edu.wm.cs.cs301.slidingpuzzle.PuzzleState#isEmpty(int,int)}. Test
	 * checks all positions on the initial state for being empty or not. Test
	 * requires setToInitialState() and isEmpty() being properly implemented.
	 */
	@Test
	 ...
	 * Trivial test to see if getStateWithShortestPath() returns a state for trivial
	 * sequences of length 0 and 1. Test is not(!) rigid enough to see if an
	 * existing cycle is recognized and removed by the method. Test requires
	 * setToInitialState() and isEmpty() being properly implemented.
	 */
	@Test
